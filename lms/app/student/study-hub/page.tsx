  ...
  return (
    <Card>
      <CardContent className="p-4 space-y-3">
        <p className="font-medium text-sm"><span className="text-muted-foreground mr-2">{idx + 1}.</span>{item.question}</p>
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/ai/study", {
        ...
        body: JSON.stringify({ topic, gradeLevel, goal }),
      });
        ...
        <CardContent className="p-5">
          <form onSubmit={handleGenerate} className="space-y-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="font-sora font-semibold">Generate Study Package</span>
            </div>
