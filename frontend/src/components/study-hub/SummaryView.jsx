...
import { Copy, RefreshCw, Download, Save } from "lucide-react";
...
export default function SummaryView({ content, streaming, onCopy, onRegenerate, onDownload, onSave }) {
  return (
    <div className="rounded-2xl border border-hub-border bg-hub-surface shadow-[0_0_0_1px_var(--hub-border),0_8px_40px_rgba(0,0,0,0.08)]">
      <OutputToolbar onCopy={onCopy} onRegenerate={onRegenerate} onDownload={() => onDownload?.(content)} onSave={onSave} />
...
export function OutputToolbar({ onCopy, onRegenerate, onDownload, onSave }) {
  return (
