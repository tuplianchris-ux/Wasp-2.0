...
import { getTests, getMyTestAttempts, submitTestAttempt } from "wasp/client/operations";
...
export function StudentTestsPage({ user }: { user: AuthUser }) {
  const { data: tests = [], isLoading: loadingT, refetch } = useQuery(getTests, { published: true });
  const { data: myAttempts = [], isLoading: loadingA } = useQuery(getMyTestAttempts);
  const [screen, setScreen] = useState<Screen>("list");
  const [activeTest, setActiveTest] = useState<any | null>(null);
  const [answers, setAnswers] = useState<Record<string, string>>({});
  const [submitting, setSubmitting] = useState(false);
  const [result, setResult] = useState<{ score: number; feedbackArr: any[] } | null>(null);

  const getStorageKey = (id: string) => `test_draft_${id}`;
  const isLoading = loadingT || loadingA;

  const getAttempt = (testId: string) =>
    (myAttempts as any[]).find((a) => a.testId === testId);

  const startTest = (test: any) => {
    setActiveTest(test);
    const saved = localStorage.getItem(getStorageKey(test.id));
  ...
  const handleAnswer = (qId: string, val: string) => {
    ...
    setAnswers(updated);
    if (activeTest) localStorage.setItem(getStorageKey(activeTest.id), JSON.stringify(updated));
  };
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const data = await submitTestAttempt({ testId: activeTest.id, answers }) as any;
  ...
  if (isLoading) {
    return (
      <AppShell role="STUDENT" userName={(user as any).name || "Student"} pageTitle="Tests">
        <LoadingSpinner />
      </AppShell>
    );
  }
  ...
  if (screen === "taking" && activeTest) {
    ...
    return (
      <AppShell role="STUDENT" userName={(user as any).name || "Student"} pageTitle={activeTest.title}>
        <div className="space-y-4 max-w-3xl">
          ...
          {activeTest.timeLimit && <TimerBar durationMinutes={activeTest.timeLimit} onExpire={handleSubmit} />}
                ...
                <Card className={answers[q.id] ? "border-primary/40" : ""}>
                  <CardContent className="p-5 space-y-3">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <Badge variant="secondary" className="shrink-0">{(q.points ?? 1)}pt{(q.points ?? 1) > 1 ? "s" : ""}</Badge>
                    </div>
  ...
  if (screen === "result" && result && activeTest) {
    const questions: Question[] = JSON.parse(activeTest.questions);
    return (
      <AppShell role="STUDENT" userName={(user as any).name || "Student"} pageTitle="Results">
        <div className="space-y-6 max-w-2xl">
          <Button variant="ghost" size="sm" onClick={() => setScreen("list")}>
            <ChevronLeft className="h-4 w-4" /> Back to Tests
          </Button>
          <div className="text-center space-y-2">
            <h2 className="text-2xl font-bold" style={{ fontFamily: "Sora, sans-serif" }}>Test Complete!</h2>
            <p className="text-muted-foreground">{activeTest.title}</p>
          </div>
          <GradeDisplay score={result.score} />
          <div className="space-y-3">
            <h3 className="font-semibold" style={{ fontFamily: "Sora, sans-serif" }}>Question Breakdown</h3>
            {questions.map((q, idx) => {
  ...
  return (
    <AppShell role="STUDENT" userName={(user as any).name || "Student"} pageTitle="Tests & Quizzes">
      <div className="space-y-6">
        <div>
          <h2 className="page-title">Tests & Quizzes</h2>
          <p className="page-subtitle">Take published assessments</p>
        </div>

        {(tests as any[]).length === 0 ? (
          <EmptyState message="No tests available yet" />
        ) : (
          <div className="space-y-3">
                      ...
                      <div className="shrink-0">
                        {attempt ? (
                          <div className="flex items-center gap-2">
                            ...
                            <span className={`font-bold ${gradeColor(attempt.score ?? 0)}`} style={{ fontFamily: "Sora, sans-serif" }}>
