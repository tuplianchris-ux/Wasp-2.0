    ...
    <div className="h-screen flex flex-col bg-background">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-card/50 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm" className="gap-2 border-border hover:border-primary/40 transition-colors">
                <Layout className="w-4 h-4 text-primary/70" />
                <span className="hidden sm:inline">Templates</span>
                <ChevronDown className="w-3 h-3 text-muted-foreground" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-[480px] p-3" sideOffset={8}>
              ...
              <div className="grid grid-cols-3 gap-2">
                {getTemplateList().map((template) => {
                  ...
                  const colorMap = {
                    blank: 'hover:border-gray-400',
                    cornell: 'hover:border-indigo-400',
                    mindmap: 'hover:border-purple-400',
                    flowchart: 'hover:border-green-500',
                    timeline: 'hover:border-violet-500',
                    comparison: 'hover:border-blue-400',
                  };
                  const selectedColorMap = {
                    blank: 'border-gray-400 bg-gray-50',
                    cornell: 'border-indigo-400 bg-indigo-50/50',
                    mindmap: 'border-purple-400 bg-purple-50/50',
                    flowchart: 'border-green-500 bg-green-50/50',
                    timeline: 'border-violet-500 bg-violet-50/50',
                    comparison: 'border-blue-400 bg-blue-50/50',
                  };
                  return (
                    <DropdownMenuItem
                      key={template.id}
                      ...
                      className={`
                        flex flex-col items-start gap-1.5 p-2 rounded-lg border cursor-pointer
                        transition-all duration-150 focus:bg-transparent
                        ${isSelected
                          ? `${selectedColorMap[template.id] || 'border-primary bg-primary/5'}`
                          : `border-border bg-card ${colorMap[template.id] || 'hover:border-primary/40'}`
                        }
                      `}
                    >
                      {/* Thumbnail */}
                      <div className="relative w-full aspect-[8/5] rounded overflow-hidden bg-white border border-border/50 shadow-sm">
                        <img
                      ...
                      <div className="w-full">
                        <p className={`text-xs font-semibold leading-tight ${isSelected ? 'text-foreground' : 'text-foreground/80'}`}>
                          {template.name}
                        </p>
      ...
      <div className="flex-1 overflow-hidden">
        <PanelGroup direction="horizontal" className="h-full">
          ...
          {/* Right Panel - Drawing Canvas (60%) */}
          <Panel defaultSize={60} minSize={50} maxSize={70}>
