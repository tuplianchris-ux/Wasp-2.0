...
export const tasksAPI = {
  ...
  getStats: async () => {
    if (USE_REAL_API) {
      ...
      const user = await dataService.getCurrentUser();
      const tasks = await dataService.getTasks(user.user_id);
...
export const gamificationAPI = {
  getStats: async () => {
    if (USE_REAL_API) {
      ...
      const xpForCurrent = Array.from({ length: level - 1 }, (_, i) => 100 + i * 50).reduce((a, b) => a + b, 0);
      const xpForNext = 100 + (level - 1) * 50;
