...
export default function StudyHub() {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      const result = useMock
        ...
        : await callAnthropic(mode, opts, text, attachments);
  ...
  }, [input, attachments, mode, getOptionsForMode, useMock]);
  ...
  const handleRegenerateMessage = useCallback(
    async (assistantMessageId) => {
      ...
      try {
        ...
        const result = useMock
          ...
          : await callAnthropic(currentMode, optsWithModel, prevMsg.content, prevMsg.attachments || []);
        const contentForMessage = result.type === "summary" || result.type === "notes" ? result.data : null;
        setMessages((prev) =>
          prev.map((m) =>
            m.id === loadingId
    ...
    [messages, mode, options, useMock, model]
  );
